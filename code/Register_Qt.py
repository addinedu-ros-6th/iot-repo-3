...
#     MyDB.commit()
    ...
    def DBConnect(self):
        ...
        with db:
            cur = db.cursor()
            #수정예정 - 이미지파일 Qt로부터 불러와서 DB에 저장
            cur.execute("INSERT INTO User VALUES ('%s','%s','%s','%s','%s')" % (self.nameEdit.text(),self.uidEdit.text(),self.floorEdit.text(),self.companyEdit.text(),self.image))
            db.commit()
